...
1. 打开 Copilot Chat 面板：`Ctrl+Alt+I`（Windows/Linux）/ `Cmd+Alt+I`（Mac）
2. 点击聊天面板顶部的 Agent 下拉菜单
3. 从列表中选择一个 `superpower-*` Agent
4. 输入你的请求并发送
...
| `superpower-context` | "我需要重构 auth 模块，涉及多个文件" |
| `superpower-mentor` | "帮我理解中间件链是怎么工作的" |
...
产出文档的 Agent（brainstorm、ui-design、context、plan）会将输出保存到工作区的 `.github/superpower/` 目录：
...
Agent 之间会自动发现彼此的产物：
- `superpower-plan` 从 `brainstorm/` 和 `context/` 读取输入
- `superpower-execute` 从 `plan/` 读取计划
- `superpower-verify` 和 `superpower-review` 从 `plan/` 读取需求

### Playwright MCP（可选）

对于需要浏览器交互的 Agent（ui-design、debug、verify、execute、tdd、review），可以选择配置 [Playwright MCP](https://github.com/anthropics/playwright-mcp) 服务器。在 `.vscode/settings.json` 中添加：
...
这会启用浏览器自动化工具（`playwright/*`），用于 UI 测试、视觉验证和前端调试。如果未配置，Agent 正常工作，只是没有浏览器能力。
